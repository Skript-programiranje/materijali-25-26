...
```
gde je `REGEX` regularni izraz kojim pretražujemo `ulaz` koji predstavjla
...
*Napomena*: Da bismo uključili podršku za više operatora u regularnim izrazima,
gotovo uvek ćemo pozivati *grep* sa opcijom `-E` koja uključuje podršku za
regularne izraze sa proširenim skupom operatora.

Primer: Komanda
```bash
grep -E '[0-9]+' ulaz.txt
```
ispisuje sve linije fajla *ulaz.txt* koje sadrži brojeve.

*Napomena*: *grep* ne podržava skraćenicu `\d` za karaktersku klasu za cifre,
zato je neophodno pisati `[0-9]`.

*Napomena*: Da bismo videli šta je tačno poklopljeno na liniji, korisna
opcija je `--color` koja boji tekst koji je poklopljen regularnim izrazom.
